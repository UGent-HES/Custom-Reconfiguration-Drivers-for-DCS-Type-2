...
u32 presort(const lutlocation *location_array,
        u8 *newtruthtables,
	...
	while(num_sorted != number_of_tluts) {
		...
		for(; i<number_of_tluts; i++) {
			if(is_sorted[i])
				continue;
			if(MiCAP_Custom_IsSameFrame(lut_configs[num_sorted-1].slice_row, lut_configs[num_sorted-1].slice_col,
...
void reconfigure_group(LUT_config_type *lut_configs,
        u32 *group_sizes) {
	...
	for(i=0;group_sizes[i]!=0;i++) {
		XStatus Status;
		Status = MiCAP_Custom_SetMultiClbBits(lut_config_p, group_sizes[i]);
		if (Status != XST_SUCCESS)
			xil_printf("MiCAP_Custom_SetMultiClbBits failure!! \n\r");
...
void preload_frames(LUT_config_type *lut_configs,	u32 *group_sizes, u32 *config_buffer) {
	...
	for(i=0;group_sizes[i]!=0;i++) {
		XStatus Status;
		Status = MiCAP_Custom_ReadClbFrames(lut_config_p, config_buffer_p);
		assert(Status == XST_SUCCESS);
		lut_config_p += group_sizes[i];
		//xil_printf("preload frames group %d\n\r", i);
		config_buffer_p += MiCAP_Custom_GetClbFramesSize();
	}
}

//reconfigure one instance with preloaded configuration frames
void reconfigure_group_with_frames(LUT_config_type *lut_configs,	u32 *group_sizes, u32 *config_buffer) {
	//reconfigure each group of TLUTs one by one
	u32 i;
	LUT_config_type *lut_config_p = lut_configs;
	u32 *config_buffer_p = config_buffer;
	for(i=0;group_sizes[i]!=0;i++) {
		XStatus Status;
		Status = MiCAP_Custom_SetMultiClbBitsWithFrames(lut_config_p, group_sizes[i], config_buffer_p);
		assert(Status == XST_SUCCESS);
		lut_config_p += group_sizes[i];
		//xil_printf("reconfig group %d\n\r", i);
		config_buffer_p += MiCAP_Custom_GetClbFramesSize();
...
void test_fir() {
	...
	u32 num_groups = presort((lutlocation*)location_array, (u8*)output, lut_configs, group_sizes, NUMBER_OF_INSTANCES*NUMBER_OF_TLUTS_PER_INSTANCE);
	xil_printf("After presort.. \n\r\n\r");
	
	xil_printf("Before preload.. \n\r\n\r");
	u32 config_frames[num_groups * MiCAP_Custom_GetClbFramesSize()];
	preload_frames(lut_configs, group_sizes, config_frames);
	...
	Start_Timer();
    start1 = Get_Timer();
	//reconfigure_group(lut_configs, group_sizes);
	reconfigure_group_with_frames(lut_configs, group_sizes, config_frames);
    stop1= Stop_Timer();
    exec1 = stop1 - start1;
