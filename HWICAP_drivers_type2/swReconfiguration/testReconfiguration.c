...
void copyLutConfig(LUT_config_type *lut_config, const lutlocation *location, u8 newtruthtable[LUT_CONFIG_WIDTH]) {
	lut_config->slice_col = location->lutCol;
	lut_config->slice_row = location->lutRow;
	lut_config->Resource = XHI_CLB_LUT_replacement.CONTENTS[location->sliceType][location->lutType];
	lut_config->NumBits = LUT_CONFIG_WIDTH;
	lut_config->Value = newtruthtable;
}

u32 presort(XHwIcap *HwIcap,
        const lutlocation *location_array,
        u8 *newtruthtables,
		LUT_config_type *lut_configs,
		u32 *group_sizes,
		u32 number_of_tluts) {

	u8 is_sorted[number_of_tluts];
	memset(is_sorted, 0x0, sizeof(u8)*number_of_tluts);

	u32 num_sorted = 0;
	u8 group_num = 0;
	u32 i;
	while(num_sorted != number_of_tluts) {
		for(i = 0; i<number_of_tluts; i++) {
			if(!is_sorted[i])
				break;
		}
		assert(i != number_of_tluts);
		group_num++;
		num_sorted++;
		group_sizes[group_num-1] = 1;
		copyLutConfig(&lut_configs[num_sorted-1], &location_array[i], &newtruthtables[i]);
		is_sorted[i] = 1;
		for(; i<number_of_tluts; i++) {
			if(is_sorted[i])
				continue;
			if(XHwIcap_Custom_IsSameFrame(HwIcap, lut_configs[num_sorted-1].slice_row, lut_configs[num_sorted-1].slice_col,
					location_array[i].lutRow, location_array[i].lutCol)) {
				num_sorted++;
	...
	group_sizes[group_num] = 0;
	return group_num;
}

//reconfigure one instance
void reconfigure_group(XHwIcap *HwIcap,
        LUT_config_type *lut_configs,
        u32 *group_sizes) {
	//reconfigure each group of TLUTs one by one
	u32 i;
	LUT_config_type *lut_config_p = lut_configs;
	for(i=0;group_sizes[i]!=0;i++) {
		XStatus Status;
		Status = XHwIcap_Custom_SetMultiClbBits(HwIcap, lut_config_p, group_sizes[i]);
		if (Status != XST_SUCCESS)
			xil_printf("XHwIcap_Custom_SetMultiClbBits failure!! \n\r");
		lut_config_p += group_sizes[i];
		//xil_printf("reconfig group %d\n\r", i);
	}
}

//preload configuration frames
void preload_frames(XHwIcap *HwIcap, LUT_config_type *lut_configs,	u32 *group_sizes, u32 *config_buffer) {
	//load the configuration frames of each group one by one
	u32 i;
	LUT_config_type *lut_config_p = lut_configs;
	u32 *config_buffer_p = config_buffer;
	for(i=0;group_sizes[i]!=0;i++) {
		XStatus Status;
		Status = XHwIcap_Custom_ReadClbFrames(HwIcap, lut_config_p, config_buffer_p);
		assert(Status == XST_SUCCESS);
		lut_config_p += group_sizes[i];
		//xil_printf("preload frames group %d\n\r", i);
		config_buffer_p += XHwIcap_Custom_GetClbFramesSize(HwIcap);
	}
}

//reconfigure one instance with preloaded configuration frames
void reconfigure_group_with_frames(XHwIcap *HwIcap, LUT_config_type *lut_configs,	u32 *group_sizes, u32 *config_buffer) {
	//reconfigure each group of TLUTs one by one
	u32 i;
	LUT_config_type *lut_config_p = lut_configs;
	u32 *config_buffer_p = config_buffer;
	for(i=0;group_sizes[i]!=0;i++) {
		XStatus Status;
		Status = XHwIcap_Custom_SetMultiClbBitsWithFrames(HwIcap, lut_config_p, group_sizes[i], config_buffer_p);
		assert(Status == XST_SUCCESS);
		lut_config_p += group_sizes[i];
		//xil_printf("reconfig group %d\n\r", i);
		config_buffer_p += XHwIcap_Custom_GetClbFramesSize(HwIcap);
...
void test_fir(XHwIcap *HwIcap_p) {
	...
	xil_printf("Before presort.. \n\r\n\r");
	u32 num_groups = presort(HwIcap_p, (lutlocation*)location_array, (u8*)output, lut_configs, group_sizes, NUMBER_OF_INSTANCES*NUMBER_OF_TLUTS_PER_INSTANCE);
	xil_printf("After presort.. \n\r\n\r");
	
	xil_printf("Before preload.. \n\r\n\r");
	u32 config_frames[num_groups * XHwIcap_Custom_GetClbFramesSize(HwIcap_p)];
	preload_frames(HwIcap_p, lut_configs, group_sizes, config_frames);
	xil_printf("After preload.. \n\r\n\r");
	...
	xil_printf("Starting FIR test round 1. . . \n\r\n\r");
		//Disable the clock
	Xil_Out32(XPAR_AXI_FIR_DIRECT_0_BASEADDR+8, clk_disable);

	//Reset the FIR filter
	Xil_Out32(XPAR_AXI_FIR_DIRECT_0_BASEADDR+12, rst_en);

	//Read the filter output
	for(i=0; i<256; i++){
		Xil_Out32(XPAR_AXI_FIR_DIRECT_0_BASEADDR, input_samples[i]);
		output_samples_hardware[i] = Xil_In32(XPAR_AXI_FIR_DIRECT_0_BASEADDR+4);
		if (output_samples_hardware[i] != 0)
			xil_printf("Filter is not Reset.. Error\n\r\n\r");
	}
		//Simulate the Hardware implemented FIR using micro-reconfiguration
	//Disable the clock
	Xil_Out32(XPAR_AXI_FIR_DIRECT_0_BASEADDR+8, clk_disable);
	...
	//Simulate the Hardware implemented FIR using micro-reconfiguration
	for(i = 0; i<31; i++){
		setParameterbits(cdata1[i],NUMBER_OF_PARAMETERS,parameter);
		evaluate(parameter,output[i]);
		i = i + 1;
		evaluate(parameter,output[i]);
	}
	...
	Start_Timer();
    start1 = Get_Timer();
	//reconfigure_group(HwIcap_p, lut_configs, group_sizes);
	reconfigure_group_with_frames(HwIcap_p, lut_configs, group_sizes, config_frames);
    stop1= Stop_Timer();
    exec1 = stop1 - start1;
    xil_printf("Reconfigure 384 TLUTs = %d\n\r\n\r", exec1);
	//Remove the reset
	Xil_Out32(XPAR_AXI_FIR_DIRECT_0_BASEADDR+12, rst_disable);

	//Enable the clock
	Xil_Out32(XPAR_AXI_FIR_DIRECT_0_BASEADDR+8, clk_en);

	xil_printf("Simulating the Hardware FIR, fetching the output samples....\n\r\n\r");

	for(i=0; i<32; i++){
		Xil_Out32(XPAR_AXI_FIR_DIRECT_0_BASEADDR, input_samples[i]);
		output_samples_hardware[i] = Xil_In32(XPAR_AXI_FIR_DIRECT_0_BASEADDR+4);
		xil_printf("output_samples_hardware = %d \n\r\n\r",output_samples_hardware[i]);
	}

	xil_printf("Starting FIR test round 2. . . \n\r\n\r");
			//Disbale the clock
		Xil_Out32(XPAR_AXI_FIR_DIRECT_0_BASEADDR+8, clk_disable);
		...
		//Simulate the Hardware implemented FIR using micro-reconfiguration
			for(i = 0; i<31; i++){
				setParameterbits(cdata3[i],NUMBER_OF_PARAMETERS,parameter);
				evaluate(parameter,output[i]);
				i = i + 1;
				evaluate(parameter,output[i]);
			}
		...
		//	presort(HwIcap_p, (lutlocation*)location_array, (u8*)output, lut_configs, group_sizes, 384);
			xil_printf("After presort.. \n\r\n\r");
